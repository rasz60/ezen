...
public class CalendarEx03 {
	...
	public static void main(String[] args) {
		...
		long distance = Math.abs(time2.getTimeInMillis() - time1.getTimeInMillis()) / 1000;
		System.out.println("time1과 time2의 차이는 " + distance + "초 입니다.");
		
		String tmp = "";
		
		for ( int i = 0; i < TIME_UNIT.length; i++ ) {
			tmp += (distance / TIME_UNIT[i]) + TIME_UNIT_NAME[i];
			distance %= TIME_UNIT[i];
		}
		
		System.out.println("시간으로 변환하면 " + tmp + "입니다.");
